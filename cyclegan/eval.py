...
class ImageDataset(Dataset):
    ...
    def download(dataset_name: str):
        ...
        url = 'https://jbox.sjtu.edu.cn:10081/v2%2Fdelivery%2Fdata%2F4f39a26535704e2082378d96bb2e129b%2F?'
        # Download folder
...
def train():
    ...
    ## Train Cycle GAN
    """
    trained_run_uuid = 'e245f45f892e11eeb31bf077c319d74a'
...
def plot_image(img: torch.Tensor, save_path='save.png'):
    """
...
def plot_images_grid(imgs):
    ...
    fig, axes = plt.subplots(num_rows, num_cols, figsize=(12, 12))  # Adjust the figsize as per your needs
...
def evaluate(start_pos):
    ...
    trained_run_uuid = '15e3ddfaa14c11ee9bb25cff35c18842'
    ...
    with experiment.start():
        ...
        # subfolders_starting_with_test = [item for item in items if os.path.isdir(os.path.join(folder_path, item)) and item.startswith("test_e98c174738")]
        subfolders_starting_with_test = [item for item in items if os.path.isdir(os.path.join(folder_path, item)) and item == "my_test"]

        # 打印满足条件的子文件夹列表
        print(subfolders_starting_with_test)
        img_list = subfolders_starting_with_test
        count = 1
        total_img = len(img_list)
        for test in img_list:
            
            dataset = ImageDataset(test, transforms_, 'test')
            ...
            for k in range(11, 12):
                ...
                save_path = 'data/cycle_gan/{}/Final{}.jpg'.format(test,test)

                plot_image(generated_y[0].cpu(), save_path)
            print("完成了{}/{}".format(count, total_img))
            count += 1
        #list_1 = []
        #list_2 = []
        #list_3 = []
        ...
        #for i in range(start_pos, start_pos + 2):
            ...
            # Display the generated image.
            #y_image = generated_y[0].cpu()

            #if i % 2 == 0:
                #list_1.append(x_image)
                #list_1.append(y_image)
            #else:
                #list_2.append(x_image)
                #list_2.append(y_image) 
            
               
               

        #image_grid = [list_1, list_2]
        #plot_images_grid(image_grid)
def original_evaluate():
    """
    ## Evaluate trained Cycle GAN
    """
    # Set the run UUID from the training run
    trained_run_uuid = '11a6408a8abc11eebdaa5cff35c18db0'
    # Create configs object
    conf = Configs()
    # Create experiment
    experiment.create(name='cycle_gan_inference')
    ...
    with experiment.start():
        ...
        # You can use `conf.dataset_name`, if you specified `dataset_name` as something you wanted to be calculated
        # in the call to `experiment.configs`
        dataset = ImageDataset('real2mural_mini', transforms_, 'train')
        # Get an image from dataset
        x_image = dataset[0]['x']
        # Display the image
        plot_image(x_image)
